...
def load_data(
    ...
    logger.info("  載入 blacklist_analysis  %d 筆", len(bl))
    logger.info("  載入 white_to_black      %d 筆", len(w2b))
    logger.info("  user_info 合併去重後     %d 筆", len(user_info))
    ...
    dfs = {}
    for stem, key in tx_files.items():
        candidates = [
            data_dir / f"{stem}.csv",
    ...
    gnn_edge = pd.read_csv(gnn_edge_path)
    gnn_node = pd.read_csv(gnn_node_path)
    logger.info("  載入 gnn_edge_list       %d 筆", len(gnn_edge))
    logger.info("  載入 gnn_node_list       %d 筆", len(gnn_node))

    return dfs["crypto_transfer"], dfs["twd_transfer"], user_info, gnn_edge, gnn_node
...
def main(
    ...
    if not skip_cxgnn:
        ...
        # 所有節點均為詐騙節點（來自 blacklist_analysis + white_to_black）
